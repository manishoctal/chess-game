      ...
      return (
        <>
          <div className={wrapperClassName || 'relative z-0 mb-6 w-full group'}>
            <select
              id={name}
              className={`${inputClass || 'block py-4 px-3 w-full text-sm text-gray-900 bg-transparent border-2 rounded-lg border-[#DFDFDF] appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0  peer'}`}
              placeholder=' '
              {...register}
              {...rest}
              disabled={disable}
