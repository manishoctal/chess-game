...
export default function ImageUploader({ onFileChange, defaultImage }) {
  ...
  return (
    <div className="w-full max-w-lg mx-auto mt-3 p-6 bg-white rounded-lg shadow-lg">
      <div className="mb-4">
        <label className="block text-gray-700 text-sm font-bold mb-2">Upload Banner Image (16:9 Aspect Ratio)</label>
        <div
        ...
        >
          ...
          <input id="fileInput" type="file" className="hidden" accept="image/*" onChange={handleChange} />
        </div>
      </div>
      {imageSrc && (
        <div className="mt-4 relative">
          ...
          <button className="absolute top-2 right-2 bg-white rounded-full p-1 shadow-md hover:bg-gray-100" onClick={removeImage}>
