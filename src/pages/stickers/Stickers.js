...
import AddEditSticker from "./AddEditSticker";
...
function Stickers() {
  ...
  const MemoizedAddEditSound = React.memo(AddEditSticker);
  ...
  const [soundEditModal, setStickerModal] = useState(false);
  const [stickers, setAllStickers] = useState([]);
  ...
  const [userSuggestion, setStickersSuggestion] = useState();
  const [searchTerm, setSearchTerm] = useState("");
  const [isInitialized, setIsInitialized] = useState(false);
  const [isSearchVisible, setIsSearchVisible] = useState(false);
  const [debouncedSearchTerm, setDebouncedSearchTerm] = useState("");
  const manager =
    user?.permission?.find((e) => e.manager === "stickers_manager") ?? {};
  const getAllStickers = async (data) => {
    try {
      const { startDate, endDate, searchkey, isFilter } = filterData;
      if (data?.deletePage || (isFilter && data?.statusChange)) {
      ...
      const path = apiPath.stickers;
      const result = await apiGet(path, payload);
      if (result?.status === 200) {
        const response = result?.data?.results;
        setAllStickers(response?.docs);
        setPaginationObj({
          ...paginationObj,
          page: response?.page,
  ...
  const getUserSuggestion = async () => {
    try {
      ...
      if (result?.status === 200) {
        const response = result?.data?.results;
        setStickersSuggestion(response?.docs);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const handleSearchKey = (e) => {
    setFilterData({ ...filterData, searchkey: e.name, isFilter: true });
    setPage(1);
  ...
  useEffect(() => {
    updatePageName(t("O_STICKERS_MANAGEMENT"));
  }, []);
