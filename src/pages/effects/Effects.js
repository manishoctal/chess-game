...
import AddEditEffects from "./AddEditEffects";
...
function Effects() {
  ...
  const MemoizedAddEditSound = React.memo(AddEditEffects);
  ...
  const [effectEditModal, setEffectEditModal] = useState(false);
  const [effect, setAllEffect] = useState([]);
  ...
  const manager = user?.permission?.find((e) => e.manager === "effects") ?? {};

  const getAllEffect = async (data) => {
    try {
      const { used, startDate, endDate, searchkey, isFilter } = filterData;
      if (data?.deletePage || (isFilter && used && data?.statusChang)) {
        if (effect?.length > 1) {
          setPage(page - 1);
          setIsDelete(true);
      ...
      const path = apiPath.effects;
      const result = await apiGet(path, payload);
      if (result?.status === 200) {
        const response = result?.data?.results;
        setAllEffect(response?.docs);
        setPaginationObj({
          ...paginationObj,
          page: response?.page,
  ...
  const getUserSuggestion = async () => {
    try {
      const path = apiPath.effectListSuggestion;
      const payload = {
        keyword: searchTerm,
        limit: 5,
      };
  ...
  const viewDetailsUser = async (newItem) => {
    try {
      const path = `${apiPath.effects}/${newItem?._id}`;
  ...
  useEffect(() => {
    updatePageName(t("O_EFFECTS_MANAGEMENT"));
  }, []);
  ...
  return (
    <div>
      ...
      <>
        ...
        {effectEditModal && item ? (
          <MemoizedAddEditSound
            setEffectEditModal={setEffectEditModal}
            onClose={() => {
              setEffectEditModal(false);
              setItem("");
            }}
            getAllEffect={getAllEffect}
            item={item}
