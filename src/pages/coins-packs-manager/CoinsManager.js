...
function CoinsManager() {
  ...
  const manager =
    user?.permission?.find((e) => e.manager === "coins_pack_manager") ?? {};
  ...
  const allCoinsListing = async () => {
    try {
      ...
      const path = apiPath.getCoinListing;
      const result = await apiGet(path, payload);
      const response = result?.data?.results?.docs;
      const resultStatus = result?.data?.success;
      setCoinListing(response);
      setPaginationObj({
        ...paginationObj,
  ...
  const handelStatusChange = async (item) => {
    try {
      ...
      const path = `${apiPath.changeCoinPackStatus}/${item?._id}`;
        ...
        <div className="px-3 py-4">
          <div className="bg-white border border-[#E9EDF9] rounded-lg dark:bg-slate-800 dark:border-[#ffffff38]">
                ...
                <div className="flex items-center lg:pt-0 pt-3 flex-wrap justify-center mb-2 2xl:mb-0">
                  <div className="relative flex items-center mb-3">
                    ...
                    <input
                      ...
                      placeholder={t("SEARCH_BY_PACKS_NAME")}
                      value={searchTerm}
                      title=""
                      required
              ...
              <div className="flex items-center justify-end px-4 ms-auto mb-3">
                {(manager?.add || user?.role === "admin") && (
                  <button
                    ...
                    onClick={() => navigate("/coins_pack_manager/add")}
                  >
                    + {t("ADD_PACKS")}
                  </button>
                )}
            ...
            <Table
              ...
              allCoinsListing={allCoinsListing}
              page={page}
              setSort={setSort}
