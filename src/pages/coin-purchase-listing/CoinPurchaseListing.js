...
function CoinPurchaseListing() {
    const { t } = useTranslation();
    const { user } = useContext(AuthContext);
    const manager = user?.permission?.find((e) => e.manager === "coin_purchase_listing") ?? {};
    const [coinPurchaseListing, setCoinPurchaseListing] = useState([]);
    ...
    const allCoinsPurchaseListing = async () => {
        try {
            ...
            const path = apiPath.coinPurchaseListing;
            const result = await apiGet(path, payload);
            const response = result?.data?.results?.docs;
            const resultStatus = result?.data?.success;
            setCoinPurchaseListing(response);
            setPaginationObj({
                ...paginationObj,
            ...
            <div className="bg-[#F9F9F9] dark:bg-slate-900">
                <div className="px-3 py-4">
                    <div className="bg-white border border-[#E9EDF9] rounded-lg dark:bg-slate-800 dark:border-[#ffffff38]">
                        ...
                        <Table
                            coinPurchaseListing={coinPurchaseListing}
                            allCoinsPurchaseListing={allCoinsPurchaseListing}
                            page={page}
                            setSort={setSort}
                            sort={sort}
                            manager={manager}
                        ...
                        />
                        {toggle && <CoinPurchaseView onHide={handleView} viewData={viewData} />}
...
export default CoinPurchaseListing