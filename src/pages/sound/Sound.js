...
function Sound() {
  ...
  const MemoizedAddEditSound = React.memo(AddEditSound);
  ...
  const [soundEditModal, setSoundEditModal] = useState(false);
  const [sounds, setAllSound] = useState([]);
  ...
  const manager =
    user?.permission?.find((e) => e.manager === "sounds_manager") ?? {};
  const getAllSound = async (data) => {
    try {
      ...
      const payload = {
        ...
        playListName: JSON.stringify(selected),
      };

      const path = apiPath.getSound;
      const result = await apiGet(path, payload);
      if (result?.status === 200) {
        const response = result?.data?.results;
        setAllSound(response?.docs);
        setPaginationObj({
          ...paginationObj,
          page: response?.page,
  ...
  const getUserSuggestion = async () => {
    try {
      ...
      if (result?.status === 200) {
        const response = result?.data?.results;
        setUserSuggestion(response?.docs);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const viewDetailsUser = async (viewItem) => {
    try {
      const path = `${apiPath.getSound}/${viewItem?._id}`;
  ...
  const handleUserView = (userItem) => {
    viewDetailsUser(userItem);
    setSoundEditModal(true);
  };

  useEffect(() => {
    getAllSound();
  }, [page, filterData, sort, pageSize, selected]);

  const handleSearchKey = (e) => {
    setFilterData({ ...filterData, searchkey: e.artistName, isFilter: true });
    setPage(1);
    setSearchTerm(e.artistName);
  ...
  useEffect(() => {
    updatePageName(t("O_SOUNDS_MANAGEMENT"));
  }, []);
  useEffect(() => {
    if (!isInitialized) {
  ...
  return (
    <div>
        ...
        <div className="px-3 py-4">
          <div className="bg-white border border-[#E9EDF9] rounded-lg">
            <form className="border-b border-b-[#E3E3E3] flex justify-between px-4 py-3 pt-5">
              ...
              <div className="flex items-center md:justify-end px-4 mb-3">
                <label
                  ...
                  <div className="relative " ref={searchRef}>
                    <div className="relative ">
                      ...
                      <input
                        ...
                        placeholder="Search by artists’ Name"
                        value={searchTerm}
                        title=""
                        required
                    ...
                    {isSearchVisible && searchTerm && sounds?.length !== 0 && (
                ...
                {(manager?.add || user?.role === "admin") && (
                  <button
                    ...
                    type="button"
                    className="w-[160px] bg-gradientTo text-sm px-8 ml-3 mb-0 py-2 rounded-lg items-center border border-transparent text-white hover:bg-DarkBlue sm:w-auto w-1/2"
                  >
                    + Add sound
                  </button>
                )}
      ...
      <>
        ...
        {soundEditModal && item ? (
          <MemoizedAddEditSound
            setSoundEditModal={setSoundEditModal}
            onClose={() => setSoundEditModal(false)}
            getAllSound={getAllSound}
            item={item}
            type="edit"
          />
