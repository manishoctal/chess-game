  ...
  return (
    <div className="p-6">
        ...
        <div className="border border-gray-300  mb-4">
          ...
          <div className="grid grid-cols-2 gap-4 mt-2 p-4">
            ...
            {helpers?.ternaryCondition(
              type === "monetary",
              <div>
                <strong>Monetary Rating: </strong>
                {helpers?.ternaryCondition(
                  item?.creatorDetails?.ratingMonetary,
                  item?.creatorDetails?.ratingMonetary,
                  0
                )}
              </div>,
              <div>
                <strong>Casual Rating: </strong>
                {helpers?.ternaryCondition(
            ...
            <div>
              ...
              {helpers?.ternaryCondition(
                item?.creatorDetails?.country,
                startCase(item?.creatorDetails?.country),
                "N/A"
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-2  p-4">
            ...
            <div className="flex items-center">
              ...
              {helpers?.ternaryCondition(
                ...
                <GiHorseHead className="text-gray-400 ml-3 text-2xl" />,
                <GiHorseHead className="ml-3 text-2xl text-black" />
              )}
            </div>
            <div>
      ...
      <div className="border overflow-hidden rounded-lg">
        <header className="border-b  py-2 px-4 bg-gray-100 rounded-t-md dark:bg-gray-800 mb-4">
          <div className="font-semibold dark:text-white">Match Detail</div>
        </header>
        ...
        <div className="overflow-x-auto relative rounded-lg ">
          <table className="w-full text-sm text-left text-[#A5A5A5] dark:text-gray-400 ">
            ...
            <tbody className="text-black">
              {type === "monetary" && (
                <td className="py-2 px-4 border-r dark:border-[#ffffff38] pl-5">
                  {helpers?.ternaryCondition(
                    item?.challengeId,
                    item?.challengeId,
                    "N/A"
                      ...
                      <div className="flex items-center justify-center">
                        ...
                        <span className="block">
                          ...
                          {helpers?.ternaryCondition(
                            ...
                            `(${item?.acceptorDetails?.ratingMonetary})`
                          )}
                        </span>
                        {helpers?.ternaryCondition(
                          showAcceptorFlag,
              ...
              <td className="py-2 px-4 border-r dark:border-[#ffffff38] text-center">
                {helpers.getDateAndTime(item?.scheduleDateTime, item?.scheduleDateTime, "N/A")}
              </td>
              ...
              {type === "monetary" && (
                <>
                  <td className="py-2 px-4 border-r dark:border-[#ffffff38] text-center">
                    {helpers?.ternaryCondition(
                      item?.winningAmount,
                      helpers?.formattedAmount(item?.winningAmount),
                      helpers?.formattedAmount(0)
                    )}
                  </td>
                  <td className="py-2 px-4 border-r dark:border-[#ffffff38] text-center">
                    {helpers?.ternaryCondition(
                      item?.adminMoneyCommission,
                      helpers?.formattedAmount(item?.adminMoneyCommission),
                      helpers?.formattedAmount(0)
                    )}
                  </td>
                </>
              ...
              <td className="py-2 px-4 border-r dark:border-[#ffffff38] text-center">
                <button
                  className="bg-gradientTo p-3 rounded-md text-white text-sm"
                  disabled={ item?.totalMoves < 1 ? true : false }
                  onClick={() =>
                    navigate("/challenges-manager/view/review-move", {
                      state: { id: item?._id },
                    })
                  }
                >
