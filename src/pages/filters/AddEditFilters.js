...
const AddEditFilters = ({ onClose, getAllFilter, item, type }) => {
  ...
  const handleSubmitForm = async (data) => {
    ...
    try {
      ...
      if (imageEffect) {
        ...
        const fetchData = await response.blob()
        const metadata = {
          type: 'videofx/*'
        }
        const file = new window.File([fetchData], `${image}.videofx`, metadata)
  ...
  const getWidgetId = async () => {
    try {
      ...
      const result = await apiGet(path, { contentType: 'Filters' })
      if (result.data.success) {
  ...
  useEffect(() => {
    if (widget && type === 'edit') {
      ...
      setImageEffect(item?.url || null)
      setEffectName(item?.url || null)
    }
  }, [widget])
        ...
        <div>
          ...
          <div className='relative z-0   w-full group'>
            ...
            <div className='block py-2 px-3 w-full text-sm text-gray-900 bg-transparent border-2 rounded-lg border-[#DFDFDF] appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0  peer'>
              ...
              <div className='image_title'> {imageName?.length > 17 ? imageName?.substring(0, 12) + '...' + imageName?.substring(imageName?.length - 5, imageName?.length) : imageName}</div>
            ...
            <div>
              ...
              {(image && !fileMsg) && <img
                src={image !== null ? image : URL.createObjectURL(watch('passportWatch')[0])}
                className='mt-5'
    ...
    return (
      <>
        ...
        <div>
          <label
            for='floating_file'
          >Upload filter <span className='text-slate-500'>(.arscene, .videofx)</span><span className='text-red-500'>*</span>
          </label>
          <div className='relative z-0   w-full group'>
            ...
            <div className='block py-2 px-3 w-full text-sm text-gray-900 bg-transparent border-2 rounded-lg border-[#DFDFDF] appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0  peer'>
              ...
              <div className='image_title'> {effectName?.length > 17 ? effectName?.substring(0, 12) + '...' + effectName?.substring(effectName?.length - 5, effectName?.length) : effectName}</div>

            </div>

            <div>
              <ErrorMessage
                message={fileMsgEffect}
              />
            </div>
          ...
          <div className='md:py-4 sm:px-2 sm:py-8 px-7'>
            <div className='border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none lg:min-w-[762px]'>
              <div className='flex items-center justify-between p-5 border-b border-solid border-slate-200 rounded-t'>
                <h3 className='text-xl font-semibold'>{type === 'add' ? 'Add' : 'Edit'} Filter</h3>
              ...
              <form onSubmit={handleSubmit(handleSubmitForm)} method='post'>
                  ...
                  <div className='grid grid-cols-2'>
                    <div className='md:py-4 sm:px-2 sm:py-3 md:px-7 px-2'>
                      <OInputField
                        ...
                        name='displayName'
                        inputLabel={<>Display name<span className='text-red-500'>*</span></>}
                        type='text'
                        autoFocus
                        maxLength={500}
                        onInput={(e) => preventMaxInput(e, 500)}
                        register={register('displayName', formValidation.displayName)}
                        errors={errors}
                        // disable={type === 'edit'}
                      />
                    </div>
                    <div className='md:py-4 sm:px-2 sm:py-3 md:px-7 px-2'>
                      <OInputField
                        wrapperClassName='relative z-0  w-full group'
                        name='duration'
                        inputLabel={<>Duration<span className='text-red-500'>*</span></>}
                        type='number'
                        maxLength={5000}
                        preventDecimal
                        // isDecimal={true}
                    ...
                    <div className='md:py-4 sm:px-2 sm:py-3 md:px-7 px-2'>
                      ...
                      >
                        ...
                        {widget
                          ? widget?.map((val) => <option value={val?._id} key={val?._id}>
                            {val?.name}
                                                       </option>)
                          : null}
                  ...
                  <div className='grid grid-cols-1'>
                    <div className='md:py-4 sm:px-2 sm:py-3 md:px-7 px-2'>
                      <OTextArea
                        wrapperClassName='relative z-0  w-full group'
                        name='description'
                        inputLabel={<>Description<span className='text-red-500'>*</span></>}
                        type='text'
                        maxLength={15000}
                ...
                <div className='flex items-center justify-center p-6 border-t border-solid border-slate-200 rounded-b'>
                  ...
                  {
                    isLoading ? <div className='spinner-container bg-LightBlue text-white active:bg-emerald-600 font-normal text-sm px-8 py-2.5 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1  ease-linear transition-all duration-150'><div className='loading-spinner' /></div> : <button className='bg-gradientTo text-white active:bg-emerald-600 font-normal text-sm px-8 py-2.5 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1  ease-linear transition-all duration-150' type='submit' onClick={() => { fileValid(); fileValidEffect() }}> {type === 'add' ? 'Add' : 'Edit'}</button>
                  }
                </div>
              </form>
            </div>
