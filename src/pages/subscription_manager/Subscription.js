...
function Subscription() {
  ...
  const [setViewShow] = useState(false);
  const [setEditShow] = useState(false);
  ...
  const allSubscription = async (data) => {
    try {
      ...
      const path = apiPath.subscription;
  ...
  const handleUserView = ({ userItem, type }) => {
    if (type === "edit") {
      setItem(userItem);
      setEditShow(true);
    } else if (type === "view") {
              ...
              <div className="flex items-center justify-end px-4 ms-auto mb-3">
                {(manager?.add || user?.role === "admin") && (
                  <button
                    ...
                    onClick={() => navigate("/subscription-manager/add")}
                  >
                    + {t("ADD_SUBSCRIPTION")}
                  </button>
                )}
