...
const Profile = () => {
  ...
  const handleStatusProfile = async () => {
    try {
      setToggleNotification(toggleNotification === "false" ? "true" : "false");
      const payload = {
      ...
      <div className="px-3 py-4">
        <div className="bg-white border border-[#E9EDF9] dark:border-[#E9EDF9] rounded-lg py-4 dark:bg-slate-800 ">
          <form onSubmit={handleSubmit(handleSubmitForm)} method="post">
            <div className="pr-8 py-4 text-right">
              <span className="block mb-3"> {t("NOTIFICATION_SETTING")}</span>
              <label
                className="inline-flex relative items-center cursor-pointer"
                title={`${
                  toggleNotification?.status === "true" ? "True" : "False"
                }`}
              >
                <input
                  type="checkbox"
                  className="sr-only peer"
                  checked={toggleNotification === "true"}
                  onChange={(e) =>
