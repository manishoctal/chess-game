...
function Category () {
  ...
  const manager = user?.permission?.find(e => e.manager === 'subscription_manager') ?? {}
  ...
  const [filterData, setFilterData] = useState({ category: '', searchkey: '', startDate: '', endDate: '', isReset: false, isFilter: false })
  ...
  const allCategory = async (data) => {
    try {
      ...
      if ((data?.deletePage && !(subscription?.length > 1) || (isFilter && category && data?.statusChange && !(subscription?.length > 1)))) {
        setPage(page - 1)
        setIsDelete(true)
      ...
      const path = apiPath.categoryAllList
  ...
  const handleUserView = ({ item, type }) => {
    if (type === 'edit') {
      setItem(item)
      setEditShow(true)
    } else if (type === 'add') {
  ...
  const handleReset = () => {
    setFilterData({ category: '', searchkey: '', startDate: '', endDate: '', isReset: true, isFilter: false })
    setPage(1)
    setSearchTerm('')
  }
  ...
  const handleDateChange = (start, end) => {
    setPage(1)
    setFilterData({ ...filterData, startDate: start, endDate: end, isFilter: true })
  }
  const statusPage = (e) => {
    setPage(1)
    ...
    <div>
                ...
                <div className='flex items-center lg:pt-0 pt-3 flex-wrap justify-center mb-2 2xl:mb-0'>
                  <div className='relative flex items-center mb-3'>
                    ...
                    <input
                      ...
                      placeholder={t('SEARCH_BY_SUB_ADMIN_NAME_ID')}
                      value={searchTerm}
                      title=''
                      required
                ...
                {(manager?.add ||
                  user?.role === 'admin') && (
                    <button
                      ...
                      onClick={() => handleUserView({ type: 'add', item: '' })}
                    >
      ...
      <>
        ...
        }
        {
          editShow
            ? (
              <AddEditCategory allCategory={allCategory} item={item} type='edit' onClose={() => setEditShow(false)} />)
            : null
}
      </>
    </div>
