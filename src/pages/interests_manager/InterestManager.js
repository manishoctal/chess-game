...
import AddEditInterest from "./AddEditInterest";
...
function InterestManager() {
  ...
  const manager =
    user?.permission?.find((e) => e.manager === "interests_manager") ?? {};
  ...
  const [filterData, setFilterData] = useState({
    category: pageFilter?.selectOne,
    managerType: DashboardStatus || pageFilter?.selectThree || "artist",
    subscription: pageFilter?.selectTwo,
    searchkey: pageFilter?.keyword,
    startDate: pageFilter?.startDate,
    endDate: pageFilter?.endDate,
    isReset: false,
    isFilter: false,
  });
  ...
  const InterestListing = async (data, pageNo) => {
    try {
      const {
        category,
        startDate,
  ...
  const handelStatusChange = async (items) => {
    try {
      const payload = {
        status: items?.status === "inactive" ? "active" : "inactive",
      };
      const path = `${apiPath.changeInterestStatus}/${items?._id}`;
      const result = await apiPut(path, payload);
      if (result?.status === 200) {
        notification.success(result.data.message);
        InterestListing({ statusChange: 1 });
      }
    } catch (error) {
                ...
                <div className="flex items-center lg:pt-0 pt-3 flex-wrap justify-center mb-2 2xl:mb-0">
                  <div className="relative flex items-center mb-3">
                    ...
                    <input
                      ...
                      placeholder={t("O_SEARCH") + " by " + t("INTERESTS_TAG")}
                      value={searchTerm}
                      title=""
                      required
                      onChange={(e) => {
