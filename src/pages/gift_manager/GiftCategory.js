...
import AddEditGifts from "./AddEditGifts";
...
function GiftCategory() {
  ...
  useEffect(() => {
    allCategory();
  }, [filterData, page, sort, categoryToggle, pageSize]);
                  ...
                  <>
                    {(manager?.add || user?.role === "admin") && (
                      <button
                        ...
                        className="bg-gradientTo flex text-sm px-8 ml-3 py-2 rounded-lg items-center border border-transparent text-white hover:bg-DarkBlue whitespace-nowrap"
                        onClick={() => {
                          navigate("/gift-manager/add");
                        }}
                        // onClick={() => handleAddGift({ type: "add", item: "" })}
                      >
                        + {t("ADD_GIFT")}
                      </button>
                    )}
