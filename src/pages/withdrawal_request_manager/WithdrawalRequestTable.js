...
const WithdrawalRequestTable = ({ allCommunity, paginationObj, sort, setSort, pageSize, getAllWithdrawal }) => {
    ...
    const handleReject = async (rejectItem) => {
        try {
            const payload = {
                status: "rejected"
            }
            const path = apiPath.rejectWithdrawalReqeust + "/" + currencyCode;
                                ...
                                return (
                                    <tr key={i} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td className="py-4 px-6 border-r text-center">
                                            {helpers.formattedAmount(reqeustedAmount)}
                                        </td>
                                        <td className="py-4 px-6 border-r text-center">
                                            {helpers.formattedAmount(remainingAmount)}
                                        </td>
                                        ...
                                        <td className="py-4 px-3 border-r text-center ">
                                            ...
                                            {
                                                item?.status === 'pending' && getTimeDifference(item?.expiryTime)
                                            }
                                        ...
                                        <td className="py-4 px-6 border-r text-center">
                                            {
                                                helpers?.ternaryCondition(item?.status === 'pending', <button onClick={(e) =>
                                                    helpers.alertFunction(
                                                        `${t("ARE_YOU_SURE_YOU_WANT_TO")} Reject?`,
                                                        item,
                                                        handleReject
                                                    )
                                                } className="bg-red-600 text-sm px-4 flex gap-2 ml-3 mb-3 py-2 rounded-lg items-center border border-transparent text-white hover:bg-red-600 sm:w-auto w-1/2">Reject</button>, "")
                                            }
