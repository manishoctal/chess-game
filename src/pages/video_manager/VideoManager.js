...
function VideoManager() {
  ...
  const manager =
    user?.permission?.find((e) => e.manager === "artwork_manager") ?? {};
  ...
  const handleCopy = (item) => {
    console.log("heyaaaaa",item?.link)
    setCopyText(item?.link);
    copy(copyText);
    alert(`Video link copied`)
  }

  const videoListing = async () => {
    try {
      ...
      const path = apiPath.videoListing;
      ...
      <div className="bg-[#F9F9F9] dark:bg-slate-900">
        <div className="px-3 py-4">
          <div className="bg-white border border-[#E9EDF9] rounded-lg dark:bg-slate-800 dark:border-[#ffffff38]">
            ...
            {toggle && <ViewVideo onHide={handleToggle} viewData={viewData} />}
          </div>
        </div>
      </div>
    </div>
